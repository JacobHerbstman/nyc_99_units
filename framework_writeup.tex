...
A developer chooses total units $m$ and a feasible division $\mathbf n=(n_1,\ldots,n_J)$, with $m=\sum_{k=1}^J n_k$. We take the parent site's total land footprint as given in both the policy and counterfactual choices. The developer chooses size and organization within that footprint. The reweighted historical joint distribution supplies ordinary size and organization, including parents that ordinarily have multiple constituents. Before the modeled escalation, surplus from total units is
\begin{equation}
...
We treat the observed parent boundary as the land available under both policy regimes. Historical land area and area-weighted FARs describe that given footprint; the number and sizes of constituents can change within it. Holding the footprint fixed is a maintained assumption about the developer's opportunity. Recent pre-policy holdouts assess whether the weighted comparison reproduces size and organization before the policy change. Floor area, stories, and other physical outcomes provide further evidence where measurement is comparable.
