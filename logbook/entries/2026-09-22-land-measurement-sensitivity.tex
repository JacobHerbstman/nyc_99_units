...
\subsection*{September 23 follow-up: symmetric merger windows and implausible sites}

Jacob approved testing the land rule symmetrically before changing production.
Two windows count the earlier-parcel area only when every DOF change on the
parent's lots was recorded within 180 or 365 days after filing, in both
periods, and keep only post-period parents observed for the full window (229
and 115 of 310). Each is compared with production land on the same sample.

\begin{center}
\small
\begin{tabular}{lrrrrr}
\hline
Scenario & Post parents & Excess at 99 & Deficit 100--149 & Hist.\ mean & Post mean \\
\hline
Production, 180-day sample & 229 & 11.2 & 8.2 & 153 & 142 \\
Mergers within 180 days & 229 & 11.2 & 8.0 & 149 & 142 \\
Production, 365-day sample & 115 & 11.7 & 8.5 & 154 & 145 \\
Mergers within 365 days & 115 & 11.7 & 8.4 & 149 & 145 \\
\hline
\end{tabular}
\end{center}

Applied the same way in both periods, the merger correction lowers the
weighted historical mean by four to five units and the 100--149 deficit by at
most 0.2 points, and leaves the excess at 99 unchanged. The 25-unit drop in the
one-sided correction was therefore an artifact of recording time, not evidence
that the production measure is biased between periods. The production measure
stands. A 365-day window catches only about half the historical mergers, so
these windows bound, rather than eliminate, the remaining undercount.

A symmetric screen flags sites whose permitted residential floor (lot area
times the larger of residential and broad FAR) is under 150 square feet per
proposed unit: 62 of 595 historical and 25 of 310 post-period parents, similar
shares. The flagged set is not neutral in outcome, however. It contains 7 of the
37 post-period parents at exactly 99 units (19 percent, against 7 percent of
other post-period parents) and none of the three historical ones; an example is
67 4th Avenue, 99 units on 1,745 square feet. Dropping these parents lowers the
excess at 99 to 9.9 points, still inside the bootstrap interval, but that is a
selection on the outcome, not a correction. The finding matters for any model
that uses a parent's own land: recorded land is most often a fragment for the
99-unit filings that identify the notch response. Claude recommends carrying
the implausible-site flag into estimation and checking results without those
parents, rather than further case-by-case research.

\textit{Reproduction note.} The window and screen scenarios are in
\path{land_measurement_sensitivity.R}; root \texttt{make land-sensitivity}.

\subsection*{September 23: the 180-day merger rule in production}

Jacob chose the 180-day window and asked that estimation keep as many
observations as possible. Production land now adds, for every filing whose
reference is a citywide MapPLUTO release, the lots DOF merged into that filing
lot after the reference map and no more than 180 days after the parent's first
filing, using their area, FARs and existing floor from the same release. Only
lot mergers with one surviving lot count. The rule lives in
\path{build_parent_site_characteristics}, reading DOF lot actions from
\path{fetch_dof_tax_map_history}; no audit output enters production. In the
weighting sample it adds lots to 63 historical and 37 post-period parents. Units,
filings and membership are unchanged, and the constituent panel is identical.

Nothing is dropped. The 81 post-period parents filed less than 180 days before
the DOF snapshot keep \texttt{merger\_window\_complete = FALSE}, and 47
historical and 21 post-period parents carry \texttt{implausible\_site}; seven of
the latter are exact-99 parents.

\begin{center}
\small
\begin{tabular}{lrrrrrr}
\hline
Scenario & Hist. & Post & Excess at 99 & Deficit 100--149 & Hist.\ mean & Post mean \\
\hline
Production (180-day rule) & 595 & 310 & 11.4 & 7.3 & 146 & 137 \\
Complete 180-day windows only & 595 & 229 & 11.2 & 8.0 & 150 & 142 \\
Without implausible sites & 548 & 289 & 9.8 & 5.9 & 135 & 136 \\
\hline
\end{tabular}
\end{center}

The benchmark effective sample size is 527; the bootstrap interval for the
excess at 99 is 7.9--14.9 points and for the 100--149 deficit 3.0--12.1. The
excess at 99 holds without the parents lacking a full window and without the
implausible sites, though removing the latter lowers it to 9.8 because it
drops seven post-period 99-unit parents. The deficit just above 100 is more
sensitive to which parents enter.
