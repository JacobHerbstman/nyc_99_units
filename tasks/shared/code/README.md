...
A main task Makefile contains the execution include, `all` listing actual outputs, the generic include, output recipes, and plain `ln -sf $< $@` input links. Scripts, task-local helpers, input links and the task Makefile are concrete prerequisites. Shared helpers in this directory are sourced without being listed; after changing one, rebuild the affected tasks with `make -B`. Release lists and patterns represent real file families. `generic.make` supplies directory rules and `OOPR`, the standard order-only prerequisites.
...
Research specifications that define a product, such as the comparison periods, are written in the tracked recipe; fixed values used by one script are written in that script. Change the recipe or script to change a specification at the same output filename. Specifications encoded in filenames, such as parcel releases and plot measures, have separate targets.
