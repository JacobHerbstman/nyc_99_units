...
`composition_eligible`. Boundary
flags and the alternative earlier-parcel areas come from `parent_site_scope.csv`
in `audit_parent_site_boundaries`.

Each scenario recalibrates the historical weights under three matches
(`match`): production zoning and borough (residential FAR, borough); the same
plus log lot area (`with_lot_area`); and the earlier match on log lot area,
residential FAR, built FAR and borough (`previous_match`). Land scenarios move
the result only through the two robustness matches. Each row reports the
exact-99 and exact-198 excesses, the 100–149 cumulative deficit,
multiple-constituent shares, and mean parent units:
