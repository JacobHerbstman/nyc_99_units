...
The output maps the BBL reported in current PLUTO to its official `APPBBL`, the apportionment BBL used by PLUTO for lots created through merges, splits, or condominium conversions. This task only creates the crosswalk. It does not decide which HDB rows should use APPBBL features; that decision happens in `build_hdb_mappluto_site_panel`.
