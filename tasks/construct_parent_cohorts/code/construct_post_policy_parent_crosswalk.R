...
# 2022 through 2026 with at least six units, counted as in the parents: the
# 25Q4 Housing Database Class A count, or the DOB count where HDB has none.
# 2022-2024 filings are padding, so companions of early 2025 filings can be
# found.
...
hdb_units <- read_parquet("../input/dcp_housing_database_project_level_25q4.parquet") |>
  select(job_number, hdb_units = classa_prop)
stopifnot(!anyDuplicated(hdb_units$job_number))
owner_placeholders <- c("N/A", "NA", "NONE", "NOT APPLICABLE")
filings <- read_parquet("../input/dob_now_new_building_initial_filings.parquet") |>
  left_join(hdb_units, by = "job_number", relationship = "many-to-one") |>
  ...
  filter(job_type == "New Building", filing_year >= 2022L, filing_year <= 2026L,
    if_else(is.na(hdb_units), abs(proposed_dwelling_units - round(proposed_dwelling_units)) < 1e-8 &
      proposed_dwelling_units >= 6, hdb_units >= 6)) |>
