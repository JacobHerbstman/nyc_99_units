...
if (length(args) != 5L) {
  stop(
    ...
    "counterfactual maximum units, and plot minimum and maximum units."
  )
}
...
scores <- score_rows |>
  ...
  select(
    observation_id, observed_units = units, component_filings,
...
counterfactual <- tibble(
  model = "enhanced_parent_2019_2023",
...
cat("Wrote the preferred enhanced-parent no-notch model results to ../output\n")
