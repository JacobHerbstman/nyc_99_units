# setwd("/Users/jacobherbstman/Desktop/nyc_99_units/tasks/audits/prepare_acris_medium_positive_review_queue/code")
...
queue <- review_clusters |>
  filter(
    review_priority == "medium_positive_price",
    !review_cluster_id %in% manual_decisions$review_cluster_id
  ) |>
  arrange(desc(positive_doc_amount_sum), document_date, review_cluster_id) |>
  mutate(
    review_order = row_number(),
    review_block = case_when(
      review_order <= 15L ~ "block_01_top15",
      review_order <= 40L ~ "block_02_next25",
      TRUE ~ "block_03_remaining34"
    ),
...
if (!setequal(queue_ids, chatgpt_prompts_out$review_cluster_id)) {
  stop("Prompt output does not cover exactly the medium-positive review queue.")
}

write_csv_if_changed(queue, "../output/acris_medium_positive_review_queue.csv")
write_csv_if_changed(review_documents_out, "../output/acris_medium_positive_review_documents.csv")
write_csv_if_changed(review_legals_out, "../output/acris_medium_positive_review_legals.csv")
write_csv_if_changed(review_parties_out, "../output/acris_medium_positive_review_parties.csv")
write_csv_if_changed(chatgpt_prompts_out, "../output/acris_medium_positive_review_chatgpt_prompts.csv")
write_csv_if_changed(decision_template, "../output/acris_medium_positive_review_decision_template.csv")
cat("Prepared ACRIS medium-positive review queue to ../output\n")
