...
acris_site_sales_by_quarter <- acris_site_sales |>
  ...
  summarise(
    ...
    acris_strict_private_sale_event_count_q = n_distinct(event_id[event_strict_private_sale]),
    acris_broad_priced_transfer_event_count_q = n_distinct(event_id[event_broad_priced_transfer]),
    ...
    acris_strict_alloc_price_allowed_res_area_sum_q = sum(event_price_alloc_allowed_res_area[strict_price_alloc_complete], na.rm = TRUE),
    acris_strict_alloc_price_lotarea_sum_q = sum(event_price_alloc_lotarea[strict_price_alloc_complete], na.rm = TRUE),
    acris_strict_alloc_price_equal_sum_q = sum(event_price_alloc_equal[strict_price_alloc_complete], na.rm = TRUE),
    ...
    acris_strict_event_ids_q = collapse_unique(event_id[event_strict_private_sale]),
    acris_broad_event_ids_q = collapse_unique(event_id[event_broad_priced_transfer]),
    acris_primary_price_complete_event_ids_q = collapse_unique(event_id[primary_price_alloc_complete]),
    acris_strict_price_complete_event_ids_q = collapse_unique(event_id[strict_price_alloc_complete]),
...
lot_quarter_panel <- merge(opportunity_lots, quarter_rows, by = NULL) |>
  ...
  mutate(
    ...
    acris_strict_private_sale_event_count_q = coalesce(acris_strict_private_sale_event_count_q, 0L),
    acris_broad_priced_transfer_event_count_q = coalesce(acris_broad_priced_transfer_event_count_q, 0L),
    ...
    acris_strict_alloc_price_allowed_res_area_sum_q = coalesce(acris_strict_alloc_price_allowed_res_area_sum_q, 0),
    acris_strict_alloc_price_lotarea_sum_q = coalesce(acris_strict_alloc_price_lotarea_sum_q, 0),
    acris_strict_alloc_price_equal_sum_q = coalesce(acris_strict_alloc_price_equal_sum_q, 0),
    sold_acris_site_q = acris_site_sale_event_count_q > 0,
    primary_private_sale_acris_q = acris_primary_private_sale_event_count_q > 0,
    strict_private_sale_acris_q = acris_strict_private_sale_event_count_q > 0,
    broad_priced_transfer_acris_q = acris_broad_priced_transfer_event_count_q > 0,
    ...
    acris_primary_price_per_allowed_policy_res_sqft_q = if_else(
      primary_price_complete_sale_acris_q & allowed_policy_res_sqft > 0,
      acris_primary_alloc_price_allowed_res_area_sum_q / allowed_policy_res_sqft,
      NA_real_
    ),
    acris_strict_price_per_allowed_policy_res_sqft_q = if_else(
      strict_price_complete_sale_acris_q & allowed_policy_res_sqft > 0,
      acris_strict_alloc_price_allowed_res_area_sum_q / allowed_policy_res_sqft,
      NA_real_
    ),
